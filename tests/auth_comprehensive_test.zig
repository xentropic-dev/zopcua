...
test "server authentication callback registration" {
    ...
    callbacks.username_password = struct {
        fn validate(username: []const u8, password: []const u8, userdata: ?*anyopaque) bool {
            _ = userdata;
    ...
    callbacks.certificate = struct {
        fn validate(certificate: []const u8, userdata: ?*anyopaque) bool {
            _ = userdata;
    ...
    callbacks.access_control = struct {
        fn check(session_id: []const u8, node_id: []const u8, attribute_id: u32, userdata: ?*anyopaque) bool {
            _ = session_id;
            _ = node_id;
            _ = attribute_id;
            _ = userdata;
...
test "authentication memory safety" {
    ...
    const policy = try allocator.dupe(u8, "http://opcfoundation.org/UA/SecurityPolicy#Basic256Sha256");
